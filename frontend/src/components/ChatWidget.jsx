...
import { askChatbot, getChatbotHealth } from '../lib/api'
...
export default function ChatWidget() {
  ...
  useEffect(() => {
    getChatbotHealth()
      .then((d) => setOnline(d.ollama ? 'rag' : 'docs'))
  ...
  async function handleSend(e) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: 'assistant',
          text: "I couldn't reach the assistant. Start the chatbot service: cd chatbot-service && python app.py",
        },
      ])
      setOnline('offline')
    } finally {
  ...
  const statusLabel =
    online === 'rag' ? 'RAG + Ollama' : online === 'docs' ? 'Doc search' : online === 'offline' ? 'Offline' : '…'
        ...
        {open && (
          ...
          >
            <div className="chat-panel-header">
              ...
              <span className="chat-status-badge">{statusLabel}</span>
              <button
